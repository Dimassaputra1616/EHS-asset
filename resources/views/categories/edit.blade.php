        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white py-3 border-bottom-0">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h5 class="card-title mb-0 fw-bold">Edit Category</h5>
                        <p class="text-muted small mb-0">Update the classification details for {{ $category->name }}</p>
                    </div>
                </div>
            </div>
                ...
                <form action="{{ route('categories.update', $category->id) }}" method="POST">
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="name" id="name" class="form-control form-control-lg fs-6 @error('name') is-invalid @enderror" value="{{ old('name', $category->name) }}" required>
                            @error('name')
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="type" id="type" class="form-select form-select-lg fs-6 @error('type') is-invalid @enderror" required>
                                <option value="asset" {{ old('type', $category->type) == 'asset' ? 'selected' : '' }}>Fixed Asset (Hardware, Furniture)</option>
                                <option value="consumable" {{ old('type', $category->type) == 'consumable' ? 'selected' : '' }}>Consumable (Stationery, Parts)</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
